...
def log_visuals(rgb_image, birdview, speed, command, loss, pred_locations, _pred_locations, _teac_locations, size=8):
    import cv2
    import numpy as np
    ...
    for i in range(min(birdview.shape[0], size)):
        ...
        def draw_dot(canvas, i, j, color, radius=2):
            x, y = int(j), int(i)
...
def train(config):
    ...
    teacher_net = BirdEyeModelPolicy(teacher_config['model_args']['backbone'], pretrained=True, all_branch=True).to(config['device'])
